...
for retry in range(10):
    seed = 2025 + retry * 1000
    if retry > 0:
        print("  Retry %d (seed=%d)..." % (retry, seed))
...
def same_orbit(Wa, Wb, Sn, max_power=100):
...
if W2_full is None:
    print("  WARNING: Could not find W2, using identity")
    W2_full = I_full_ln
...
if dim_inter > 0:
    ...
    print("  Nontrivial!  nullspace dim = %d" % len(null_basis))
else:
...
# ================================================
print("\n" + "=" * 65)
print("  ATTACK VERIFICATION (O-known)")
print("  dim(intersection) = %d -> 16^%d candidates" % (dim_inter, q^dim_inter))
print("=" * 65)
        ...
        for k in range(o):
            ...
            if block.is_zero():
                ...
                if found_j is not None:
                    print("    block %d: S^{%d} * e  OK" % (k, found_j))
                else:
...
print("  t=3 Attack (O-known):")
print("    dim(intersection) = %d" % dim_inter)
print("    Solution found: %s" % ("OK" if solution_y1 is not None else "FAIL"))
